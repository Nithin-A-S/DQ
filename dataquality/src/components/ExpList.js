...
//        // Send the summary data for execution
//     })
//       .then((response) => response.json())
//       .then((data) => {
//         setExecutionResult(data);
...
const ExpList = () => {
  ...
  // Prevent API call if already in progress
  const handleExecute = () => {
    if (loading) return; // Prevent multiple API calls when loading is true
    ...
    fetch('http://127.0.0.1:5000/test-1', {
      ...
      .catch((error) => {
        console.error('Error executing validations:', error);
      })
      .finally(() => {
        setLoading(false); // Hide loader after execution
      });
  };
  ...
  useEffect(() => {
    if (location.state?.summaryData) {
      setSummaryData(location.state.summaryData);
      console.log("Summary data received:", location.state.summaryData);
    }
  }, [location.state?.summaryData]); // Ensure it only runs when `location.state.summaryData` changes
 
  // For logging the updated `summaryData`
  useEffect(() => {
  ...
  return (
    <div className="summary-container">
      ...
      <div className="button-group">
        ...
        <button className="nav-button" onClick={handleExecute} disabled={loading}>
      ...
      {executionResult && (
        <div className="execution-result">
          ...
          <table className="execution-table">
            ...
            {/* Uncomment the code below once execution result is processed */}
{/*             
            <tbody>
              {executionResult.validation_results.map((result, index) => (
                <tr key={index} className={result.success ? 'row success' : 'row fail'}>
